...
namespace StructuredLogViewer
{
    public class Timeline
    {
        ...
        private void Populate(Build build, bool includeCpp = false)
        {
            ...
            string regexLibFinalTime = @"^Lib: Final Total time = (?'msTime'([0-9]*\.[0-9]+|[0-9]+))s \< (?'startTime'[\d]*) - (?'endTime'[\d]*) \>\s*PB:\s*[\d]* \[(?'filename'[^\]]*)\]\s*$";
            Regex libFinalTime = new Regex(regexLibFinalTime, RegexOptions.Multiline);
            ...
            string regexLinkPass1 = @"^Cleanup phase took (?'msTime'[0-9]*.[0-9]*)ms.$";
            Regex libLinkPass1 = new Regex(regexLinkPass1, RegexOptions.Multiline);
            string regexLinkPass2 = @"^Cleanup phase took (?'msTime'[0-9]*.[0-9]*)ms.$";
            Regex libLinkPass2 = new Regex(regexLinkPass2, RegexOptions.Multiline);

            bool globalBtplus = false;
            bool globalLibTime = false;
            bool globalLinkTime = false;
